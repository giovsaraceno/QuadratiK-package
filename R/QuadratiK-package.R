#' @title Collection of Methods Constructed using the Kernel-Based Quadratic Distances
#' @docType package
#' @description
...
#' \itemize{
#'   \item \strong{Goodness-of-Fit Tests}: The software implements one, two, and k-sample tests for goodness of fit, offering an efficient and mathematically sound way to assess the fit of probability distributions. Expanded capabilities include supporting tests for uniformity on the \eqn{d}-dimensional Sphere based on Poisson kernel densities.
#'   \item \strong{Clustering Algorithm for Spherical Data}: the package incorporates a unique clustering algorithm specifically tailored for spherical data. This algorithm leverages a mixture of Poisson kernel-based densities on the Sphere, enabling effective clustering of spherical data or data that has been spherically transformed. The package also provides the functions for density evaluation and random sampling from the Poisson kernel-based distribution.
#'   \item \strong{Additional Features}: Alongside these functionalities, the software includes additional graphical functions, aiding users in validating and representing the cluster results as well as enhancing the interpretability and usability of the analysis.
#' }
#' 
#' @details
