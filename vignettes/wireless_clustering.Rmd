...
```
The `pkbc` function create an object of class `pkbc` containing the clustering results for each value of number of clusters considered. 
...
We can also request the scatter plot only for a specified value of number of clusters. Additionally, if true labels are available and are provided to the ``plot`` method, the scatter plot will display the data points colored with respect to the true labels and assigned memberships in two adjacent plots.   
